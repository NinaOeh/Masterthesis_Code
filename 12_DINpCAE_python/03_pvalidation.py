...
file_names = [
            ...
            #and "50-100" in fname
            #and 'olci' not in fname 
            #and 'multi' not in fname
            #and "20-30" not in fname and "30-40" not in fname
        ]
print(f"Pred File names: {file_names}")

file_numbers = [file_name.split(".")[0].split('_')[-1] for file_name in file_names]
...
#%%
print(np.nanmax(y_files[10]))
print(np.nanmax(y_files_trans[10]))
...
print(file_numbers)
value = '80'
index = file_numbers.index(value)
print(index)
y_pred = pred_files[index]
y_train = x_files[index]
y_true = y_files[index]

y_pred_trans = pred_files_trans[index]
y_train_trans = x_files_trans[index]
y_true_trans = y_files_trans[index]
...
for i in range(8): #len(file_numbers)
    ...
    f.suptitle(f"Model DINpCAE, {missing_perc[1:3]}% missing", fontsize = 20)    
...
generate_pred_histogram(ys, preds, f"chl_{model_name}_true", DATA_PATH_PREDICTION) # 
...
print(r2_score(val_ys[~np.isnan(val_ys)], val_preds[~np.isnan(val_preds)]),
      R_squared(val_ys[~np.isnan(val_ys)], val_preds[~np.isnan(val_preds)]))
...
print(r2_score(val_ys_real[~np.isnan(val_ys_real)], val_preds_real[~np.isnan(val_preds_real)]),
      R_squared(val_ys_real[~np.isnan(val_ys_real)], val_preds_real[~np.isnan(val_preds_real)]))
