...
EPOCHS = 1000
DATA_PATH_TRAINING = "../../CHL_DATA/TrainData_NC"
DATA_PATH_VALIDATION = "../../CHL_DATA/ValData_NC"
BATCH_SIZE = 10
...
def model_builder(hp):
    #with strategy.scope():
        model = DINpCAE(nx=dim[1], ny=dim[2], ntime=ntime_win,
                    ...
                    nvar=nvar,
                    skip="Sum", #hp.Choice("skip", ["Sum", "Concat"]),
                    stride_param=1, #hp.Choice("stride_param", [1, 0]),
                    l1=hp.Float("l1", min_value=0, max_value=0.3, step=0.005),
                    l2=hp.Float("l2", min_value=0, max_value=0.3, step=0.005)
                    )
        return model.model
    
cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath="./tune_new",
                                                 save_best_only=True)
...
tuner = kt.BayesianOptimization(
    ...
    max_trials=500,
    project_name="baysian_tuning_50_500trialsepochs2") #,distribution_strategy=strategy
