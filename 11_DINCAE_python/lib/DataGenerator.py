...
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, input_dir, batch_size=20, dim=(20,160,160), n_channels=1,
                 ...
                 jitter_std = [0.05], ntime_win=3, mean_olci = None, mean_multi = None):
        """
    ...
    def __getitem__(self, index):
        ...
        X = np.empty((self.batch_size, self.dim[1], self.dim[2], 2 * self.ntime_win + 4))
        ...
        for i in range(self.batch_size):
            ...
            # _, lambda_cb = stats.boxcox(data[~np.isnan(data)])
            # data = 10*(data**lambda_cb-1)/(lambda_cb)
...
def load_gridded_nc(fname, varname, minfrac = 0.05):
    ...
    # data = np.exp(data) - 1
    # data = (data**(-0.1745758230162425)-1)/(-0.1745758230162425)
...
#%%
def Test_Data_Loader(file, dim=(20,160,160), ntime_win=3, obs_err_std = [1.], 
    ...
    file_infos = load_gridded_nc(file,"CHL" )
    ntime = dim[0]

    X = np.empty((ntime, dim[1], dim[2], 2 * ntime_win + 4))
    ...
    time = file_infos[2]
    data =  file_infos[3]
    missing = perlin_noise_generator(dim[0],dim[1], rand_seeds, miss, train=False)
    ...
    print(np.sum(np.where(X[:,:,:,0]== 0, 0, 1))/np.sum(np.where(Y== 0, 0, 1)))
    print((np.sum(np.where(X[:,:,:,0]== 0, 0, 1)))/np.sum(np.where(mask==0, 0, 1)*20))

    return X, Y, lon, lat, time, meandata, mask
