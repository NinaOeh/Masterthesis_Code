...
class STpconv(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size = (3,3,3), delation_rate = 1, 
                 strides = (1,1,1), use_bias = True, activation = None,
                 regularizer = None, **kwargs):
      super(STpconv, self).__init__() # add name and other args?
      
      self.filters = filters
      self.kernel_size = kernel_size
      self.delation_rate = delation_rate
    ...
    def get_config(self):
      ...
      return {"filters": self.filters, 
              ...
              "delation_rate" : self.delation_rate
              } 
    ...
    def call(self, inputs):
      ...
      mask_output = tf.nn.conv3d(masks, self.kernel_mask, strides = (1,) + tuple(self.strides) + (1,), 
                                 dilations=[1, 1, self.delation_rate, self.delation_rate, 1], 
                                 padding = "VALID") # TODO: data format?
      img_output = tf.nn.conv3d(masks * imgs, self.kernel, strides = (1,) + tuple(self.strides) + (1,), 
                                dilations=[1, 1, self.delation_rate, self.delation_rate, 1], 
                                padding = "VALID") # TODO: data format?
