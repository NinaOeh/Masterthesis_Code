...
MODELNAME = "model_3_2_1_1_1_1_hybrid"
...
def create_overlapping_frames(dataset, history_size=10,
    ...
    for i in range(start_index, end_index):
        ...
        # X = np.append(X, dataset[:,i-history_size:i,:,:,5], axis=0)
        # X = np.append(X, dataset[:,i-history_size:i,:,:,8:11], axis=0)
        # X = np.append(X, dataset[:,i-history_size:i,:,:,12:], axis=0)
...
# Create overlapping windows of history and target data.
x_dataset, y_dataset = create_overlapping_frames(dataset, 10)



# In the first step we will split the data in training and remaining dataset
X_train_ind, X_rem_ind, y_train, y_rem = train_test_split(np.arange(x_dataset.shape[0]),
                                                          np.arange(x_dataset.shape[0]), 
                                                          train_size=0.8,
                                                          random_state=42)
...
x_test = x_dataset[X_test_ind]
x_val = x_dataset[X_valid_ind]
y_train = [i[X_train_ind] for i in y_dataset]
y_test = [i[X_test_ind] for i in y_dataset]
y_val = [i[X_valid_ind] for i in y_dataset]


#%%
print(f"shape of training data: {x_train.shape}")
print(f"shape of training labels: {y_train[0].shape}")
print(f"shape of validation data: {x_val.shape}")
print(f"shape of validation labels: {y_val[0].shape}")

#%%

# fill nan values with 0
x_train = np.nan_to_num(x_train)
y_train = [np.nan_to_num(y) for y in y_train]
x_val = np.nan_to_num(x_val)
y_val = [np.nan_to_num(y) for y in y_val]  
x_test = np.nan_to_num(x_test)
y_test = [np.nan_to_num(y) for y in y_test]


# Inspect the dataset.
print("Training Dataset Shapes: " + str(x_train.shape) + ", " + str(y_train[0].shape))
print("Validation Dataset Shapes: " + str(x_val.shape) + ", " + str(y_val[0].shape))
print("Test Dataset Shapes: " + str(x_test.shape) + ", " + str(y_test[0].shape))
