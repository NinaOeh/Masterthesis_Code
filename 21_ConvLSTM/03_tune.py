...
class MyHyperModel(kt.HyperModel):
    def build(self, hp):
        model = model_creator(x_train.shape[2:],
                    learning_rate=hp.Float("learning_rate", min_value=1e-5, 
                                        max_value=1e-2, step=0.00002),
                    dropout_rate=0.2, #hp.Float("dropout", min_value=0.0, max_value=0.5, step=0.1),
                    filter=2, #hp.Int("filter", min_value=0, max_value=3, step=1),
                    kernel_size_1=3, #hp.Int("kernel_size_1", min_value=3, max_value=7, step=2),
                    kernel_size_2=7, #hp.Int("kernel_size_2", min_value=3, max_value=7, step=2),
                    kernel_size_3=7, #hp.Int("kernel_size_3", min_value=3, max_value=7, step=2),
                    act="tanh", #hp.Choice("act", ["LeakyReLU", "tanh"]),
                    kernel_size_final=7, #hp.Int("kernel_size_final", min_value=3, max_value=7, step=2),
                    )
        return model
    ...
    def fit(self, hp, model, *args, **kwargs):
        return model.fit(
            *args,
            batch_size=4, #hp.Choice("batch_size", [2,4,8]),
            **kwargs,
        )
...
tuner = kt.BayesianOptimization(
    ...
    max_trials=100,
    overwrite=True,
    directory="tuning_models_LSTM",
    project_name="tune_hypermodel_50_epochs",
)
        
    
cp_callback = [tf.keras.callbacks.ModelCheckpoint(filepath="./tuning_models_LSTM/tune_hypermodel_50_epochs",
                                                 save_best_only=True),
