...
model = keras.models.load_model(f"{MODELNAME}.h5",
                                custom_objects={'SaConvLSTM2D': SaConvLSTM2D})
...
def create_overlapping_frames(dataset, history_size=10,
    ...
    for i in range(start_index, end_index):
        ...
        #X = np.append(X, dataset[:,i-history_size-target_size:i,:,:, 1:-1], axis=0)
...
all_init_frames_base = test_dataset[0][:,:,:,:,0] * (chl_max - chl_min) + chl_min
for i in range(test_dataset[0].shape[0]):
...
for idx, ax in enumerate(axes[0]):
    ...
    im1 = ax.imshow(np.squeeze(original_frames[idx_,:,:]), vmin=0, vmax=vmax, origin='lower')
...
for idx, ax in enumerate(axes[1]):
    ...
    im2 = ax.imshow(np.squeeze(frames[idx,:,:]), vmin=0, vmax=vmax, origin='lower')
    ax.tick_params(labelbottom=False, labelleft=False, length=0)
    
# Plot the Error
for idx, ax in enumerate(axes[2]):
    idx = idx + history_size-2
    im3 = ax.imshow(frames[idx,:,:]-original_frames[idx,:,:], vmin=-2, vmax=2, origin='lower', cmap='RdYlGn')
    ax.tick_params(labelbottom=False, labelleft=False, length=0)
...
def generate_residuals_figure(ys_real, preds_real, prefix, path, i):
    ...
    axes[0].hist(ys_real, bins=50, label="True", color=my_cmap(1), alpha=0.8)
    axes[0].hist(preds_real, bins=50, 
                label="Predicted", color='springgreen', alpha=0.8)
...
COLS = ['Chlorophyll-a','Cloudmask','Wind v10','Wind u10',
        'Prescipitation', 'Air pressure', 'Radiation', 'Sea surface temperature', 
...
rmses_feat = np.empty((test_dataset.shape[-1], test_dataset.shape[0], target_size))   

# get the feature importance for each feature    
for k in tqdm(range(test_dataset.shape[-1])):
    
    # SHUFFLE FEATURE K
    saved_dataset = test_dataset[:,:,:,k].copy()
    np.random.shuffle(test_dataset[:,:,:,k])
    
    for i in range(test_dataset.shape[0]):
        print(i)
        example = test_dataset[i]
        
        frames_input = example[:history_size, ...]
        
        frames = example[:history_size,:,:,0]
        original_frames = example[history_size:,:,:,0]
        ...
        # transform to original scale
        predicted_frame_orig = predicted_frame * (chl_max - chl_min) + chl_min
        #predicted_frame_orig = np.exp(predicted_frame)
        
        original_frame = original_frames * (chl_max - chl_min) + chl_min
        #original_frame = np.exp(original_frame)
        
        for j in range(target_size):        
            # calculate the rmses for the prediction per day
            rmses_feat[k,i,j] = np.sqrt(np.mean((original_frame[j,:,:] - predicted_frame_orig[j,:,:,0])**2))
           
    test_dataset[:,:,:,k] = saved_dataset
...
for i in range(5):
    df = results[i]
    print(df)
    axes[i].barh(np.arange(len(COLS)+1),df.rmses)
    axes[i].set_yticks(np.arange(len(COLS)+1),df.feature.values)
    axes[i].set_title(f'Day {i+1}',size=16)
    axes[i].set_ylim((-1,len(COLS)+1))
    axes[i].plot([baselines[i],baselines[i]],[-1,len(COLS)+1], '--', color='orange',
                label=f'Baseline \nRMSE={baselines[i]:.3f}')
    axes[i].set_xlabel(f'RMSE',size=14)
    axes[i].legend()

axes[0].set_xlim((3.15,3.17))
axes[1].set_xlim((3.28,3.3))
axes[2].set_xlim((3.4,3.41))
axes[3].set_xlim((3.6,3.615))
axes[4].set_xlim((3.615,3.630))    
...
# Construct a figure for the original and new frames.
fig, axes = plt.subplots(3, target_size+2, figsize=(20, 10))

# Plot the original frames.
for idx, ax in enumerate(axes[0]):
    idx = idx + history_size-2
    im1 = ax.imshow(np.squeeze(original_frames[idx,:,:]), vmin=0, vmax=40, origin='lower')
...
# Display the figure.
plt.colorbar(im1, ax=axes[0, :].tolist())
plt.colorbar(im2, ax=axes[1, :].tolist())
plt.colorbar(im3, ax=axes[2, :].tolist())
plt.show()
...
DATA_PATH_PREDICTION = "../DINCAE_python/data/prediction_pDINCAE_1_1_0_1_3_0_4_0_F_S_S_train_filled"
MODELNAME = "SaConvLSTM_2"
...
# load the files
pred_files = [
        os.path.join(DATA_PATH_PREDICTION, fname)
        for fname in sorted(os.listdir(DATA_PATH_PREDICTION))
        if fname.endswith(".nc") and fname.startswith("PRED_") 
    ]
dim=(20,160,160)
dataset = np.zeros((len(pred_files),dim[0],dim[1],dim[2],1),dtype="float32") # set last dimension to 5eg 

for i in range(len(pred_files)):
    dataset[i,:,:,:,:], mask = Data_Loader(pred_files[i],dim=dim)
...
# Select a random example from the validation dataset.
example = val_dataset[np.random.choice(range(len(val_dataset)), size=1)[0]]/ np.nanmax(val_dataset)
...
# Plot the original frames.
for idx, ax in enumerate(axes[0]):
    im1 = ax.imshow(np.squeeze(np.where(original_frames[idx,:,:,0] == 0, np.nan, original_frames[idx,:,:,0])), vmax=1, vmin=0)
    ax.set_title(f"Frame {idx + 11}")
    ax.axis("off")
...
for idx, ax in enumerate(axes[1]):
    im2 = ax.imshow(np.squeeze(np.where(original_frames[idx,:,:,0] == 0, np.nan, new_frames[idx,:,:,0])), vmax=1, vmin=0)
    ax.set_title(f"Frame {idx + 11}")
    ax.axis("off")
...
for example in examples:
    ...
    for frame_set in [original_frames, new_predictions]:
        ...
        # Construct a GIF from the frames.
        with io.BytesIO() as gif:
            imageio.mimsave(gif, current_frames, "GIF")
            predicted_videos.append(gif.getvalue())
